...
(defvar-local operator-known-operators-spaced (operator-setup-strg operator-known-operators-spaced-maybe)
  "Used to skip over operators at point.")

;; (setq operator-known-operators-spaced (operator-setup-strg operator-known-operators-spaced-maybe))
...
Return position."
  (save-excursion
    ...
    (forward-char -1)
    (skip-chars-backward operator-known-operators-spaced)
...
(defun operator--do-python-mode (char start pps list-start-char  &optional notfirst notsecond unary)
  "Python"
...
(defun operator--haskell-notsecond (char start pps list-start-char notfirst notsecond)
  (cond (notsecond)
	...
	((and (operator--in-list-continue-p following_start_char)
	      ;; "(september <|> oktober)"
	      (not (char-equal ?$ char)))
...
(defun operator--do-haskell-mode (char start pps list-start-char &optional notfirst notsecond)
  "Haskell"
  (let* ((notfirst (operator--haskell-notfirst char start pps list-start-char notfirst notsecond))
	 (notsecond (operator--haskell-notfirst char start pps list-start-char notfirst notsecond)))
...
(defun operator--do-intern (char)
  (let* ((pps (parse-partial-sexp (point-min) (point)))
    ...
    (pcase major-mode
      (`python-mode
       (operator--do-python-mode char start pps list-start-char notfirst notsecond))
      (`py-python-shell-mode
       (operator--do-python-mode char start pps list-start-char notfirst notsecond))
      (`py-ipython-shell-mode
       (operator--do-python-mode char start pps list-start-char notfirst notsecond))
      ;; (`emacs-lisp-mode
      ;;  (operator--do-emacs-lisp-mode char start pps list-start-char notfirst notsecond))
      (`haskell-mode
       (operator--do-haskell-mode char start pps list-start-char notfirst notsecond))
